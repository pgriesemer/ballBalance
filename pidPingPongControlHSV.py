...
TARGET_FREQ_HZ = 7   # --- Set a more realistic control loop frequency
...
class PIDController:
    ...
    def update_constants(self, kp, ki, kd):
        ...
        self.reset_integral()
        self.prev_error = 0
        print(f"PID constants updated: Kp={kp}, Ki={ki}, Kd={kd}")
...
class BallBalancerApp:
    def __init__(self, root):
        ...
        self.prev_pos_m = None # --- MODIFIED: Previous position stored in meters
        ...
        self.v_min = tk.IntVar(value=232)
        self.h_max = tk.IntVar(value=180)
        self.s_max = tk.IntVar(value=110)
        ...
        # --- MODIFIED: Target position is now in meters ---
        self.target_x_m = (RESIZE_WIDTH / 2.0) / PIXELS_PER_METER
    ...
    def create_control_window(self):
        ...
        pid_frame = ttk.LabelFrame(self.control_window, text="PID Gains")
        pid_frame.pack(padx=10, pady=5, fill=tk.X)
        for var, label in zip([self.kp_var, self.ki_var, self.kd_var], ["Kp", "Ki", "Kd"]):
            ttk.Label(pid_frame, text=label, width=4).pack(side=tk.LEFT)
            ttk.Entry(pid_frame, textvariable=var, width=8).pack(side=tk.LEFT, padx=2)
        
        button_frame = tk.Frame(pid_frame)
        button_frame.pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="Update PID", command=self.update_pid_constants).pack(fill=tk.X)
        ttk.Button(button_frame, text="Reset Integral", command=self.pid.reset_integral).pack(fill=tk.X, pady=2)
    ...
    def create_plot_window(self):
        ...
        fig, (self.ax_pos, self.ax_vel, self.ax_int) = plt.subplots(3, 1, figsize=(6, 7), dpi=100, sharex=True)
        
        # --- MODIFIED: Position plot now uses meters ---
        ...
        self.ax_pos.axhline(self.target_x_m, color='b', linestyle='--', label="Target")
        self.ax_pos.legend()
        self.ax_pos.grid(True)

        # --- MODIFIED: Velocity plot now uses m/s ---
        self.line_vel, = self.ax_vel.plot([], [], 'g-', label="Velocity (m/s)")
        self.ax_vel.set_title("Ball Velocity vs. Time")
        self.ax_vel.set_ylabel("X Velocity (m/s)")
        self.ax_vel.set_ylim(-1.0, 1.0) # Set a reasonable m/s limit
        self.ax_vel.legend()
        self.ax_vel.grid(True)
        
        # --- MODIFIED: Integral plot now uses m*s ---
    ...
    def update_pid_constants(self):
        ...
        self.pid.update_constants(kp, ki, kd)
    ...
    def run_main_loop(self):
        ...
        if self.show_video_var.get():
            target_pixel = int(self.target_x_m * PIXELS_PER_METER)
            cv2.line(frame, (target_pixel, 0), (target_pixel, frame.shape[0]), (255, 0, 0), 2)
        
        if ball_x is not None:
            self.ball_found = True
            
            # --- MODIFIED: Calculations are now done in meters ---
            ball_x_m = ball_x / PIXELS_PER_METER
            error_m = self.target_x_m - ball_x_m
            ...
            now = time.time()
            dt = now - self.pid.last_compute_time
            velocity_ms = (ball_x_m - self.prev_pos_m) / dt if (self.prev_pos_m is not None and dt > 0) else 0
            self.prev_pos_m = ball_x_m
            
            self.t_data.append(now - self.start_time)
            self.x_data.append(ball_x_m)
            self.v_data.append(velocity_ms)
        ...
        try:
            ...
            # --- FIX: Only update plot limits if there is more than one data point ---
            if len(self.t_data) > 1:
                self.ax_pos.set_xlim(self.t_data[0], self.t_data[-1])
                for ax in [self.ax_pos, self.ax_vel, self.ax_int]:
                    ax.relim()
